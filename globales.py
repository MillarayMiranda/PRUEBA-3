...
def leer_archivo_json(dir):
    try:
        with open(dir, 'r', encoding='utf-8') as archivo: # leemos el archivo
...
def guardar_archivo_json(dir, data):
    try:
        with open(dir, 'w', encoding='utf-8') as archivo: # leemos el archivo
...
def archivo_existe(dir):
    return os.path.exists(dir)

def eliminar_archivo(dir):
    if archivo_existe(dir):
        os.remove(dir)
        input("El archivo ha sido eliminado, presione ENTER para continuar")
    else:
        input("No exiten archivos para eliminar, presione ENTER para volver al menú principal")    